...
# Function to predict most likely HT/FT scoreline
def recommend_ht_ft_score(ht_probs, ft_probs, ht_odds, ft_odds):
    """Recommend the most likely HT/FT score based on odds and probabilities."""
    ht_score = np.argmax(ht_probs)  # Choose the highest probability for HT
    ft_score = np.argmax(ft_probs)  # Choose the highest probability for FT
    return f"Halftime: {['Home', 'Draw', 'Away'][ht_score]} - Fulltime: {['Home', 'Draw', 'Away'][ft_score]}"
...
# Betting Odds (Halftime and Fulltime)
...
# Calculate HT/FT Odds Probabilities
ht_probs = [1 / ht_home, 1 / ht_draw, 1 / ht_away]
ft_probs = [1 / ft_home, 1 / ft_draw, 1 / ft_away]

# Normalize HT/FT probabilities
ht_probs = np.array(ht_probs) / np.sum(ht_probs)
ft_probs = np.array(ft_probs) / np.sum(ft_probs)
...
# Recommend HT/FT Correct Score
recommendation = recommend_ht_ft_score(ht_probs, ft_probs, [ht_home, ht_draw, ht_away], [ft_home, ft_draw, ft_away])
st.subheader("Recommended HT/FT Scoreline")
